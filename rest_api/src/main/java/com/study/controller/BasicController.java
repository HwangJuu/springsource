...
public class BasicController {
	...
	//json타입 :(path = "/send", produces = MediaType.APPLICATION_JSON_VALUE)
	...
	//ResponseEntity 타입 : Http상태 코드 + 데이터
	...
	// xml 형태로 보이지 않으면 produce써주면 됨.
	@GetMapping("/product/{cat}/{pid}")
	public String[] getPath(@PathVariable("cat") String cat,@PathVariable("pid") String pid) {
		return new String[] { "category :" +cat, "productId :"+pid};
	}
	
	// @RequestBody : JSON 데이터를 서버로 가져올때 원하는 타입의 객체로 변환
	@PostMapping("/test1")
	public void test1(@RequestBody SampleDTO dto) { //keyvalue가 같아야 데이터 값을 넘길 수 있음
