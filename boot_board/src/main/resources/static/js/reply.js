 ...
 * read.jsp 댓글 스크립트
 */
//모듈화(서버 쪽의 ajax 요청을 모다 둔 함수)
// let 메소드=(function(){})(); 구조 꼭 맞추기
...
let replyService=(function(){
	...
	function add(reply, callback){ 
		...
		$.ajax({
		   ...
		   beforeSend:function(xhr){
			...
			//JSON.stringify 자바스크립트 객체인데 json으로 바꿔줘. 바꿔서 데이터로 보내줘.
	...
	// sysdate ==1651633451000나옴.
	function displayTime(timeValue){
		//현재 시간이 ms 단위로 나와있는 것을 변환
		...
		if(gap < (1000*60*60*24)){
			let hh = dateObj.getHours();// 0 ~ 9 에 해당하면 한자리로 나옴.
			...
			let mm = dateObj.getMonth()+1; //1월을 0으로 시작하기 때문에 +1 하기
	...
	//모듈화로 하면 직접적으로 외부에서 호출할수없어서 꼭 넣어줘야함.
	return {
		add:add, 
		getList: getList,
		get:get,
...
$(function(){
	...
	function showList(page){
		...
		// page:page||1 ==페이지 변수 값이 들어오면 사용하고, 안들어오면 1
		//result.replyCnt, result.list ==> total에 개수 
		replyService.getList({ bno: bno, page: page || 1 }, function(total, list) {
			//console.log(data);//콘솔창에서 data 넘어오는지 확인
	...
	//챗 영역에 이벤트 걸기 li 영역에 위임해줘 넘길 수 있음.
	//li영역을 넣었다 뺐다 하는 중
	//호출해서 하나 가지고 올거
	//.on("click",
	$(".chat").on("click", "li",function(){
		
		//사용자가 클릭한 li안에 rno 값을 가져오기
		//데이터 값 가지고 올 때 attr이 아닌 data로 호출
		let rno = $(this).data("rno");		
		
		replyService.get(rno,function(data){
			//console.log(data); 데이터 잘 왔는지 확인
	...
	$("#modalModBtn").click(function(){
		...
		//로그인 사용자와 댓글 작성자(modalInputReplyer)가 같은 사람인지 확인
