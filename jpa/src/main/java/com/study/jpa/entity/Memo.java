...
// 일반적으로 두가지(@GeneratedValue,@Id)를 같이 씀.
// @Entity가 붙은 클래스는 primary key에 해당하는 특정 필드를 @Id로 지정해야만 함
// @GeneratedValue(strategy = GenerationType.SEQUENCE, generator = "mem_seq_gen") ==> pk를 어떤 방식으로 삽입 할 것인지
// @Id : PK지정

//GenerationType.SEQUENCE : 오라클 + @SequenceGenerator 같이 사용
//GenerationType.IDENTITY : 사용하는 데이터베이스가 키 생성을 결정(MySQL, Maria)
//GenerationType.AUTO (default) : 특정 데이터베이스(MySQL, Maria)에 맞게 자동으로 생성됨
//GenerationType.TABLE : 키 생성 전용 테이블 사용, @TableGenerator 과 함께 사용
